...
"<div class=\"s\"><h3>Configurazione URL</h3>"
"<div class=\"ig\"><label>URL Stream ESP32-CAM</label>"
...
"3. L'URL sara' <code>http://IP:81/stream</code><br>"
"4. Esempio: <code>http://192.168.1.100:81/stream</code></p></div>"
"<div class=\"hint\"><h4>Risoluzione problemi</h4>"
"<p>- Verifica che la camera sia accesa e connessa al WiFi<br>"
"- Verifica che sia sulla stessa rete dell'OraQuadra<br>"
"- Prova ad aprire l'URL nel browser per verificare lo stream</p></div>"
"</div></div><script>"
"function load(){fetch('/espcam/config').then(r=>r.json()).then(d=>{"
"document.getElementById('url').value=d.url||'';"
"document.getElementById('info').textContent='URL attuale: '+(d.url||'non configurato');"
"}).catch(()=>{})}"
...
"document.getElementById('info').textContent='URL attuale: '+u}else{m.className='msg err';m.textContent='Errore salvataggio'}})"
