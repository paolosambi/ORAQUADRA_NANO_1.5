...
#define ROM_CPU1_ARR      digdug_rom_cpu1
#define ROM_CPU2_ARR      digdug_rom_cpu2
#define ROM_CPU3_ARR      digdug_rom_cpu3
#define TILEMAP_ARR       digdug_tilemap
#define SPRITES_ARR       digdug_sprites
#define PFTILES_ARR       digdug_pftiles
#define CMAP_TILES_ARR    digdug_colormap_tiles
#define CMAP_SPRITES_ARR  digdug_colormap_sprites
#define COLORMAPS_ARR     digdug_colormaps
#define PLAYFIELD_ARR     digdug_playfield
#define WAVE_ARR          digdug_wavetable
#define LOGO_ARR          digdug_logo

void digdug::reset() {
  machineBase::reset();
  sub_cpu_reset = 1;
  digdug_video_latch = 0;
  namco_command = 0;
  namco_mode = 0;
  namco_nmi_counter = 0;
  namco_credit = 0;
...
void digdug::blit_tile(short row, char col) {
  ...
  unsigned char chr = PLAYFIELD_ARR[addr + (digdug_video_latch&3)*0x400];
  unsigned char color = chr >> 4;
  const unsigned short *tile = PFTILES_ARR[chr];   // playfield

  // colorprom contains 4*16 color groups
  const unsigned short *colors =
    CMAP_TILES_ARR[(digdug_video_latch&0x30)+color];

  chr = memory[addr];
  // upper four bits point directly into the colormap
  const unsigned short *fgtile = TILEMAP_ARR[chr & 0x7f];
  unsigned short fgcol = ((unsigned short*)COLORMAPS_ARR)
    [((chr >> 4) & 0x0e) | ((chr >> 3) & 2)];

  // this mode is never used in digdug
  if(digdug_video_latch & 4)
    fgcol = ((unsigned short*)COLORMAPS_ARR)[chr & 15];
