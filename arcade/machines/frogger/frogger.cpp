...
#define ROM_CPU1_ARR    frogger_rom_cpu1
#define ROM_CPU2_ARR    frogger_rom_cpu2
#define TILEMAP_ARR     frogger_tilemap
#define SPRITES_ARR     frogger_sprites
#define COLORMAP_ARR    frogger_colormap
#define LOGO_ARR        frogger_logo
...
void frogger::wrZ80(unsigned short Addr, unsigned char Value) {
  // frogger main cpu
  if(current_cpu == 0) {
...
void frogger::outZ80(unsigned short Port, unsigned char Value) {
  if(current_cpu == 1) {
...
unsigned char frogger::inZ80(unsigned short Port) {
...
void frogger::prepare_frame(void) {
  active_sprites = 0;
...
void frogger::blit_tile(short row, char col) {
...
void frogger::blit_tile_scroll(short row, signed char col, short scroll) {
  if((row < 2) || (row >= 34))
    return;
